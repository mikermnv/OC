...
int main()
{
    ...
    {
        std::cout << "Invalid array number.\n";
        return -1;
    }

    HANDLE* markers = new HANDLE[threads_number]; //treads

    CRITICAL_SECTION cs;
    InitializeCriticalSection(&cs);

    HANDLE* handlesCannotContinue = new HANDLE[threads_number]; //events
    std::vector<HANDLE> handlesContinueWork; //events
    HANDLE* handlesEndWork = new HANDLE[threads_number]; //events
